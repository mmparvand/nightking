...
- Master:
  ```bash
  curl -fsSL https://raw.githubusercontent.com/mmparvand/nightking/main/install.sh | sudo bash
  ```
- Node:
  ```bash
  curl -fsSL https://raw.githubusercontent.com/mmparvand/nightking/main/install.sh | sudo bash -s -- --node
  ```
- Non-interactive node example:
  ```bash
  curl -fsSL https://raw.githubusercontent.com/mmparvand/nightking/main/install.sh | sudo bash -s -- --node --master-url https://panel.example.com --node-name edge-1 --location DE --token supersecrettoken
  ```
...
- Node: `scripts/node-install.sh` installs Docker/Compose, spins up the node agent + xray, and prompts for master URL, token, name, and location to register.
  - Universal installer: `install.sh` supports `--master` (default) or `--node` with optional `--master-url/--node-name/--location/--token` for non-interactive runs. Repo lives at `/opt/nightking`.
