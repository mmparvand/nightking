...
from .security import get_password_hash, verify_password, sign_payload
import httpx
import ipaddress
...
def node_status(node_id: int, db: DbDep, current_user: CurrentUser):
    ...
    if not node:
        raise HTTPException(status_code=404, detail="Node not found")
    try:
        _enforce_node_ip_allowlist(node.ip_address, settings)
...
def node_apply_config(
    ...
    try:
        ...
        signature = sign_payload(token, body, timestamp, nonce)
        _enforce_node_ip_allowlist(node.ip_address, settings)
        resp = httpx.post(
        ...
        crud.add_audit_log(db, current_user.username, "node_apply_error", f"{node_id}:{exc}")
        raise HTTPException(status_code=502, detail=f"Node apply failed: {exc}")
    crud.add_audit_log(db, current_user.username, "node_apply", f"{node_id}:success")
...
def rotate_node_token(node_id: int, db: DbDep, current_user: CurrentUser):
    if current_user.role != Role.ADMIN:
        raise HTTPException(status_code=403, detail="Admin only")
    node = crud.get_node(db, node_id)
    if not node:
    ...
    node.auth_token_hash = get_password_hash(new_token)
    db.commit()
    crud.add_audit_log(db, current_user.username, "node_rotate_token", str(node_id))
    return {"node_id": node_id, "token": new_token}


def _enforce_node_ip_allowlist(ip: str, settings):
    if not settings.node_agent_allowed_ips:
        return
    try:
        candidate = ipaddress.ip_address(ip)
        for net in settings.node_agent_allowed_ips:
            if candidate in ipaddress.ip_network(net, strict=False):
                return
    except ValueError:
        pass
    raise HTTPException(status_code=403, detail="Node IP not allowed")
